...
def gradient_descent(N_iterations=10, Learning_rate_alpha=.00001, B_0=(0,0)):
    """
    Whiteboard: Motivate xj+1 = xj − γ df dx
Problem: Find x that minimises f (x ) Solution
    1. Select a start value x0 and (small) learning rate γ 
    2. Apply repeatedly xj+1 = xj − γ df/dx
    3. Stop when |xj+1 − xj | < ε or after a fix number of iterations
